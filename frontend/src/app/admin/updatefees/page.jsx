...
export default function UpdateFeesPage() {
  ...
  const updateListingFee = async () => {
    try {
      ...
      await tx.wait();
      toast("Listing fee updated successfully");
      setNewListingFee("");
    } catch (error) {
  ...
  const updateRoyaltyFee = async () => {
    try {
      ...
      await tx.wait();
      toast("Royalty fee updated successfully");
      setNewRoyaltyFee("");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="p-8 bg-zinc-900 rounded-lg shadow-lg">
          {isConnected ? (
            <>
              <h1 className="text-2xl font-bold text-slate-300 mb-6 text-center">
                Update Fees
              </h1>
              ...
              <div className="mb-8 text-center">
                ...
                <input
                  ...
                  className="block w-full mt-2 p-2 border border-zinc-800 bg-zinc-950 text-slate-300 rounded"
                />
                <button
                  onClick={updateListingFee}
                  className="mt-2 px-4 py-2 bg-pink-500 text-white rounded hover:bg-pink-600 transition-colors"
                >
                  Update Listing Fee
                </button>
              </div>
